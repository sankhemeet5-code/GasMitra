...
export default function AIInsightsPage() {
  ...
  const sendMessage = () => {
    ...
    setMessages((prev) => [...prev, { role: "user", text: input }, { role: "ai", text: reply }]);
    setInput("");
  };

  return (
    <AppShell>
      <h1 className="mb-4 text-2xl font-semibold">AI Insights</h1>
      ...
      <Card className="mt-4">
        ...
        <CardContent>
          <div className="mb-3 max-h-64 space-y-2 overflow-y-auto rounded-lg border border-slate-800 bg-slate-950 p-3">
            {messages.length === 0 && (
              <p className="text-sm text-slate-400">Ask about demand, shortages, or fair allocation.</p>
            )}
            {messages.map((m, i) => (
              <div
                key={i}
